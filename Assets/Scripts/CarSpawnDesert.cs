...
public class CarSpawnDesert : MonoBehaviour
{
    private CarData car;
    ...
    void Update()
    {
        if (Input.GetButtonDown("Reset"))
        {
            ...
            if (check2.activeSelf)
            {
                ...
                Player.transform.rotation = Quaternion.Euler(0, -93.2f, 0);
            }
            if (check3.activeSelf)
            {
                Player.transform.position = check2.transform.position;
                Player.transform.rotation = Quaternion.Euler(0, -88.3f, 0);
            }
            ...
            if (check5.activeSelf)
            {
                ...
                Player.transform.rotation = Quaternion.Euler(0, 47.6f, 0);
            }
            if (check6.activeSelf)
            {
                Player.transform.position = check5.transform.position;
                Player.transform.rotation = Quaternion.Euler(0, 44f, 0);
            }
            if (check7.activeSelf)
            {
                Player.transform.position = check6.transform.position;
                Player.transform.rotation = Quaternion.Euler(0, -65.1f, 0);
            }
