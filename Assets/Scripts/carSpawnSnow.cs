...
public class carSpawnSnow : MonoBehaviour
{
    private CarData car;
    ...
    void Update()
    {
        if (Input.GetButtonDown("Reset"))
        {
            ...
            if (check1.activeSelf)
            {
                ...
                Player.transform.rotation = Quaternion.Euler(0, -266.1f, 0);
            }
            if (check2.activeSelf)
            {
                Player.transform.position = check1.transform.position;
                Player.transform.rotation = Quaternion.Euler(0, -273.9f, 0);
            }
            if (check3.activeSelf)
            {
                Player.transform.position = check2.transform.position;
                Player.transform.rotation = Quaternion.Euler(0, 8.2f, 0);
            }
